...
public class TestData {
	...
	List<Book> allBooks = new ArrayList<Book>() {
		{
			...
			add(new Book("28-12331", "Antartica", 7,  Arrays.asList(allAuthors.get(0), allAuthors.get(2))));
			add(new Book("99-22223", "Thinking Java", 21,  Arrays.asList(allAuthors.get(0), allAuthors.get(3))));
			add(new Book("48-56882", "Jimmy's First Day of School", 7,  Arrays.asList(allAuthors.get(0), allAuthors.get(4))));		
		}
	};
	...
	List<User> allUsers = new ArrayList<User>() {
		{
			add(new User("2", "b", Auth.LIBRARIAN));
			add(new User("1", "a", Auth.ADMIN));
			add(new User("3", "c", Auth.BOTH));
		}
	};
}
